...
const hideCssContent = showCssContent
  ...
  .replace(
    ...
    '/* Hide GitHub elements when extension is active */',
  ); // Update comment for hide CSS
...
export const HIDE_CSS_CONTENT = \`${hideCssContent.replace(/`/g, '\\`')}\`;

export const SHOW_CSS_CONTENT = \`${showCssContent.replace(/`/g, '\\`')}\`;
`;
